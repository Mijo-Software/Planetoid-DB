    ...
    public static class ClsColor
    {
        ...
        public static void DrawRoundedRectangle(Graphics g, int x, int y, int width, int height, int diameter, Color color)
        {
            using (Pen pen = new Pen(color: color))
            {
                ...
                RectangleF ArcRect = new RectangleF(location: BaseRect.Location, size: new SizeF(width: diameter, height: diameter));
                //top left Arc
                g.DrawArc(pen: pen, rect: ArcRect, startAngle: 180, sweepAngle: 90);
                g.DrawLine(pen: pen, x1: x + Convert.ToInt32(value: diameter / 2), y1: y, x2: x + width - Convert.ToInt32(value: diameter / 2), y2: y);

                // top right arc
                ArcRect.X = BaseRect.Right - diameter;
                g.DrawArc(pen: pen, rect: ArcRect, startAngle: 270, sweepAngle: 90);
                g.DrawLine(pen: pen, x1: x + width, y1: y + Convert.ToInt32(value: diameter / 2), x2: x + width, y2: y + height - Convert.ToInt32(value: diameter / 2));

                // bottom right arc
                ArcRect.Y = BaseRect.Bottom - diameter;
                g.DrawArc(pen: pen, rect: ArcRect, startAngle: 0, sweepAngle: 90);
                g.DrawLine(pen: pen, x1: x + Convert.ToInt32(value: diameter / 2), y1: y + height, x2: x + width - Convert.ToInt32(value: diameter / 2), y2: y + height);

                // bottom left arc
                ArcRect.X = BaseRect.Left;
                g.DrawArc(pen: pen, rect: ArcRect, startAngle: 90, sweepAngle: 90);
                g.DrawLine(pen: pen, x1: x, y1: y + Convert.ToInt32(value: diameter / 2), x2: x, y2: y + height - Convert.ToInt32(value: diameter / 2));
            }
        }
    }
}
