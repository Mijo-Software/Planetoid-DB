	...
	public partial class TableModeForm : KryptonForm
	{
		...
		private void ListViewTableMode_SelectedIndexChanged(object sender, EventArgs e)
		{
			if (listViewTableMode.SelectedIndices.Count > 0)
			{
				...
				if (selectedIndex >= 0)
				{
					SetStatusbar(text: $"{I10nStrings.Index}: {listViewTableMode.Items[index: selectedIndex].Text} - {listViewTableMode.Items[index: selectedIndex].SubItems[index: 1].Text}");
				}
			}
		}
		#endregion
